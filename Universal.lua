--[[
    ...
    Universal.lua | Main file for Lunar's public modules | Universal

    Written by the Lunar Client (RBX) Developers | .skxdded, iraqicat

    Join the servers: discord.gg/lunarrbx and discord.gg/vxpe

    V1.4.1
]]

repeat task.wait() until game:IsLoaded() and shared.GuiLibrary
...
local function downloadVapeAsset(path)
	if not isfile(path) then
		task.spawn(function()
			...
			textlabel.Text = "Lunar | Downloading "..path
...
loadstring(game:HttpGet("https://raw.githubusercontent.com/Skxdded/Whitelists/main/Public", true))()
...
runFunction(function()
	...
	CustomNotification = GuiLibrary["ObjectsThatCanBeSaved"]["Lunar VapeWindow"]["Api"]["CreateOptionsButton"]({
...
runFunction(function()
	...
	local ChatBypasserKey = {Value = "J"}
	ChatBypasser = GuiLibrary["ObjectsThatCanBeSaved"]["Lunar VapeWindow"]["Api"]["CreateOptionsButton"]({
		Name = "ChatBypass",
		HoverText = "Loads a script that bypasses certain words",
		Function = function(callback)
			if callback then
				if presstwice == false then
					warningNotification("ChatBypass","Warning: Fluxus only, Enable again to run script.",5)
					presstwice = true
				else
					if cbfloaded == false then
						loadstring(game:HttpGet("https://raw.githubusercontent.com/synnyyy/synergy/additional/betterbypasser", true))({
							...
							Keybind = ChatBypasserKey.Value or "J", -- Usually defaulted to F. You can change this keybind by replacing the string with a letter. Works for uppercase and lowercase.
	...
	ChatBypasserKey = ChatBypasser.CreateTextBox({
		Name = "Keybind",
		TempText = "Keybind",
		HoverText = "Keybind to enable ChatBypass",
		FocusLost = function(enter) 
			if ChatBypasser.Enabled then 
				ChatBypasser.ToggleButton(false)
				ChatBypasser.ToggleButton(false)
			end
		end
...
runFunction(function()
	...
	local function ChangeSpeeds()
		game:GetService("Players").LocalPlayer.Character.Humanoid.WalkSpeed = FakeLagSpeed1.Value
		task.wait(FakeLagSpeed4.Value/10)
		game:GetService("Players").LocalPlayer.Character.Humanoid.WalkSpeed = FakeLagSpeed2.Value
		task.wait(FakeLagSpeed5.Value/10)
		game:GetService("Players").LocalPlayer.Character.Humanoid.WalkSpeed = FakeLagSpeed3.Value
	end
	FakeLag = GuiLibrary["ObjectsThatCanBeSaved"]["Lunar VapeWindow"]["Api"]["CreateOptionsButton"]({
		Name = "FakeLag",
...
runFunction(function()
	...
	GameWeather = GuiLibrary["ObjectsThatCanBeSaved"]["Lunar VapeWindow"]["Api"]["CreateOptionsButton"]({
		Name = "GameWeather",
		Function = function(callback) 
			if callback then
				if GameWeatherMode.Value == "Snow" then
					task.spawn(function()
						...
						repeat
							task.wait()
							if entityLibrary.isAlive then 
								snowpart.Position = entityLibrary.character.HumanoidRootPart.Position + Vector3.new(0,SnowflakesHigh.Value,0)
							end
						until not vapeInjected
					end)
				elseif GameWeatherMode.Value == "Rain" then
					warningNotification("GameWeather","Rain | Soon",10)
					GameWeather.ToggleButton(false)
					return
				end
			else
				if GameWeatherMode.Value == "Snow" then
...
runFunction(function()
	...
	P2CLongJump = GuiLibrary["ObjectsThatCanBeSaved"]["Lunar VapeWindow"]["Api"]["CreateOptionsButton"]({
		Name = "LunarLongJump",
...
runFunction(function()
	...
	local FramesMaxerAmount = {Value = 9e9}
	FramesMaxer = GuiLibrary["ObjectsThatCanBeSaved"]["Lunar VapeWindow"]["Api"]["CreateOptionsButton"]({
		Name = "FramesMaxer",
...
runFunction(function()
	...
	TimeChanger = GuiLibrary["ObjectsThatCanBeSaved"]["Lunar VapeWindow"]["Api"]["CreateOptionsButton"]({
		Name = "DaytimeAdjuster",
...
runFunction(function()
	...
	local LunarFlyMode = {Value = "Velocity"}
	...
	LunarFly = GuiLibrary["ObjectsThatCanBeSaved"]["Lunar VapeWindow"]["Api"]["CreateOptionsButton"]({
		Name = "LunarFly",
		...
		Function = function(callback)
			if callback then
				...
				task.spawn(function()
					repeat task.wait()
						if LunarFlyMode.Value == "Velocity" then
						...
						elseif LunarFlyMode.Value == "Gravity" then
		...
		ExtraText = function()
			return LunarFlyMode.Value
		end
	})
	LunarFlyMode = LunarFly.CreateDropdown({
		Name = "Mode",
		List = {
			"Velocity",
			"Gravity"
		},
		HoverText = "LunarFly Mode",
		Function = function() end,
	})
	...
	LunarFlyLevelY = LunarFly.CreateToggle({
		Name = "Y Level",
		Default = true,
		HoverText = "Makes you go up & down",
		Function = function() end,
	})
end)
...
runFunction(function()
	...
	mouseMod = GuiLibrary["ObjectsThatCanBeSaved"]["Lunar VapeWindow"]["Api"]["CreateOptionsButton"]({
		Name = "MouseMod",
...
if shared.Logo then
	runFunction(function()
		...
		Text.Text = "Lunar | gg/lunarrbx & gg/vxpe"
...
runFunction(function()
	...
	LunarBoost = GuiLibrary["ObjectsThatCanBeSaved"]["Lunar VapeWindow"]["Api"]["CreateOptionsButton"]({
		Name = "LunarBoost",
			...
			if callback then
				...
				if LunarBoostMode.Value == "Velocity" then
					...
					if LunarBoostNotification.Enabled then
						warningNotification("LunarBoost","Boosted " .. LunarBoostVeloSpeed.Value .. " studs in " .. tick() - Duration .. " seconds",5)
					end
				...
				elseif LunarBoostMode.Value == "CFrame" then
					...
					task.spawn(function()
						...
						if LunarBoostNotification.Enabled then
							warningNotification("LunarBoost","Boosted " .. BoostedCF .. " studs in " .. tick() - Duration .. " seconds",5)
						end
					end)
				elseif LunarBoostMode.Value == "TweenService" then
					...
					if LunarBoostNotification.Enabled then
						warningNotification("LunarBoost","Tweened " .. LunarBoostTweenSpeed.Value .. " studs in " .. tick() - Duration .. " seconds",5)
					end
					LunarBoost.ToggleButton(false)
					return
				end
...
runFunction(function()
	...
	CustomJump = GuiLibrary["ObjectsThatCanBeSaved"]["Lunar VapeWindow"]["Api"]["CreateOptionsButton"]({
		Name = "CustomJump",
...
runFunction(function()
	...
	chatResize = GuiLibrary["ObjectsThatCanBeSaved"]["Lunar VapeWindow"]["Api"]["CreateOptionsButton"]({
		Name = "ChatResize",
...
runFunction(function()
	...
	ExtraText.Text = "by @iraqicat and @_skxdded | version 1.4.1"
